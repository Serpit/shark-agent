...
仓库以数据与记忆为主,另有少量取数脚本放在 `scripts/`(见下方「脚本」)。后续添加程序化代码时,在本文件补充对应的运行/测试说明,并同步更新 `AGENTS.md`。
...
6. 扫 `memory/talks/` 中显式标记为 `> 状态：未结转` 的最近 3 条 talk，关注是否有待行动或心理卡点标记；`已结转` 的历史 talk 不在会话启动时读取，仅在当前话题同源或需要追溯证据时按需召回
