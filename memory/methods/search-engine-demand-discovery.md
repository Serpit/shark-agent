...
> **红线的前置条件(2026-08-11 新增,先读这条再用下面的阈值)**:
> **第三方工具返回「不可用 / No keyword ideas / 无数据」时,不等于搜索量为 0,不能直接 pass。**
> 这类返回值的含义是"该词低于本工具的收录门槛",不是"没人搜"。
> 判别方法:同一次会话里查一个已知有量的词(如 `3d printing`)——若它正常返回,说明工具没坏,
> 是这个词太长尾。**此时唯一正确的处理是标记为「未知,需上线后用 GSC 验真」,而不是当 0 处理。**
>
> 实证:`split 3mf` 在 Semrush 里搜索量/KD/全球量三项全「不可用」,
> GSC 实测约 **1,180 曝光/月**——不但不是 0,还落在下方"理想区间"内。
> 详见 [experiments.md 2026-08-11 曝光量订正](../experiments.md#英文-seo-内容站矩阵起步)。

**三个红线**:
- **搜索量 < 200/月**:即使 KD=0 也撑不起流量,pass。**仅在工具给出了具体数字时适用**——
  返回"不可用"走上方前置条件,不适用本红线
...
#### 补充探针:`intitle` 供给密度 / KGR(2026-08-11 加入,🚧 **待验证,不作 GO 依据**)

> **来源:哥飞,他人观点,尚未在本项目任何一个词上验证过。**
> 完整辩证与不采纳项见 [`advice/2026-08-11-gefei-kgr-intitle.md`](../advice/2026-08-11-gefei-kgr-intitle.md)。
> 回测通过前,本节只用于**给候选排序**,不得单独否决或放行任何词。

**动作**:Google 搜 `intitle:"完整关键词"`(必须带双引号),记结果数。数字不显示时点 Tools 按钮。
**读什么**:有多少页面**专门为这个词做了标题** —— 即供给端竞争密度。
这是现行 Step 2/3 完全没测的维度:Step 3 看的是 SERP 前 10 的**质量**(大厂还是 listicle),这条看的是**数量**。

`KGR = intitle 结果数 ÷ 月搜索量`,原始阈值 < 0.25 判蓝海。**本项目按数量级读(0.0X / 0.X / >1),不卡 0.25。**

**四条使用约束(与原方法的差异,都有理由,不要简化掉)**:

1. **不卡 0.25 硬阈值** —— 分子是 Google 的估算值(同词翻页都会变),撑不起这个精度;且 0.25 校准自 2017 年的 `allintitle` 口径,换算符换年代都没重算
2. **原方法只适用于月搜索量 < 250 的词** —— 这条常被转述时丢掉。**脱离该约束套到高量词上已经不是 KGR 了**,此时结果只能当粗略参考
3. **分母取不到时不强行算比值** —— 极长尾工具词在 Semrush / Ahrefs 常返回"不可用"(见上方前置条件),此时**只看 intitle 绝对值**判供给密度,KGR 留空
4. **必须排在意图红线之后** —— KGR 只测竞争密度,**完全不测变现**。一个 how-to 词可以供给稀薄、KGR 极低,却 93% informational(`astrocartography` 长尾就是);`minecraft skin generator` 880/月 · KD 30 · **CPC $0** 同理。**先过三条红线,再用 KGR 排序**

**回测计划(未做,做完才摘掉 🚧)**:对 4 个已知结局的站的主攻词跑 intitle ——
`split 3mf` / `depixelate` / `all wishes come true` / `framdrop`,看当初的 KGR 能否分辨出
"跑出量的" 与 "几乎无量的"。这是把外部观点转成自有方法论的唯一合法路径。
