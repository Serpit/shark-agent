...
## 第五站候选筛选与付费意愿验证(M5 前置,2026-08-26)

- **状态**:running
- **关联方向**:[themes.md 支付引荐扫描](themes.md#支付引荐扫描品类与候选2026-08-26-首轮)
- **待验证假设**:支付引荐池的 6 个候选里,至少有 1 个方向同时满足:①核心产品词有付费意愿(CPC / 竞品收入)②产品做在市场单独付费的层级③能在 2 周内通过定向触达验证付费,值得升级为第五站 MVP。
- **时间盒**:桌面筛选 3 天;候选定标后另开 48 小时付费探针。**前置阶段不写产品代码。**
- **预算上限**:桌面筛选 $0;付费探针落地页与 refundable deposit 最多 $10 现金成本。
- **成功标准(GO)**:至少 1 个候选通过三道门,并在 15 个定向触达内拿到 **≥3 个非熟人有效邮箱 + ≥1 笔 $19 refundable deposit**;之后才批准注册域名与两周 MVP。
- **失败标准(NO-GO)**:6 个候选全部在付费层 / 可复制分发 / 两周验证任一门被否,或定标候选触达 15 人后 0 deposit;本轮停止,不因已投入调研时间而开发。
- **执行边界**:第五站不是“再建一个能排名的站”,而是第一次先验付费再开发。第三方流量只用于排序;支付引荐访问不等于成交;最终 GO 只认真实 deposit。

### 执行队列

- [x] `outrank.so`——人工开站 + 支付信号 + 产品词 / 渠道 / 公开收入对质
- [x] `forgegui.com`——真正收费层与获客结构已确认;不进入付费探针
- [x] `trustmrr.com`——目录 / 数据产品的付费层与双平台信号已确认;不进入付费探针
- [x] `adspirer.ai`——产品本体实际是跨广告平台的 MCP 操作代理;不进入直接克隆探针
- [x] `app.tryatria.com`——产品本体、定价和 Stripe + Paddle 双平台信号已确认;不进入付费探针
- [x] `3daistudio.com`——仅作为“免费转换器是 AI 3D 生成漏斗”的竞品情报,不进入第五站候选

### 结果记录

- **2026-08-26 · `outrank.so`:高价值对标,但不直接克隆(数据源:SimilarWeb 引荐表 / 网站分析 / 关键词概览,均为第三方估算;官网与 Indie Hackers 公开案例实读)**
  - **钱确实在流**:Stripe 引荐位次 **957→307**,引荐访问 **4.8K→17.9K**;整站 2026-02~07 约 **11.5万→16.6万/月**。官网公开价 **$99/月**(原价锚点 $200),30 篇/月,叠加关键词研究、自动发布、backlink exchange、多站折扣。
  - **卖的不是“文章”,是“SEO 结果托管”**:从关键词计划到每日发布、内外链、站群和 backlink network 全包。市场单独为这一层付费,通过“做在被定价为 0 的哪一层”三问。
  - **但它不是靠产品词 SEO 起量**:SimilarWeb 2026-05~07 渠道为直接 **48.83%**、自然搜索 **19.80%**、自然社媒 **9.93%**、付费搜索 **7.79%**、外链 **6.61%**;自然搜索里品牌词 **51%**。Top 非品牌词主要是 `google alternatives` / crawler simulator / 免费工具等,与付费产品不同源。
  - **产品词也不支持“照抄后靠 SEO 冷启动”**:SimilarWeb 2026-07 显示 `ai seo tool` 约 **1,600/月、KD 82、CPC $0.32–1.60、100% informational**;`ai seo content generator` 约 **650/月**,CPC 区间虽到 $21.60 但体量小且 68% zero-click;`seo content automation` 约 **100/月、CPC $0**。这些读数只能作排序,但足以否定“靠一个大产品词冷启动”。
  - **不可复制 boost 已被公开案例坐实**:Indie Hackers 记录创始人 Eugene 的 v1 做了数月、上线后卡在 **$400 MRR**;与 Tibo 建立战略合作、补分发后才到 **>$83K/月 / 七位数 ARR**。这不是当前“X 仅 1 条 + 无分发合伙人”的同阶段样本。
  - **五重过滤**:同模式 `部分通过`(订阅 SaaS,但交付范围远超两周 MVP);同阶段 `失败`;同合规 `可解`(Waffo 可收款);同需求层 `通过`(用户为 SEO 增长结果付费);同验证 `失败`(战略合伙人、既有受众、联盟 / backlink network 无法剥离)。
  - **判定**:**不做 Outrank 克隆,不注册域名。** 保留一个窄化方向作为后续候选题:把自有四站反复踩过的 canonical / redirect / sitemap / soft-404 / 发布漏项做成“AI 建站发布防错 + 持续巡检”付费层;它必须靠 15 个目标用户的 deposit 验真,不能拿 Outrank 的收入外推。

- **2026-08-26 · `forgegui.com`:付费层成立,但增长来自游戏开发生态而非可复制产品词(数据源:支付引荐表 / SimilarWeb 关键词概览,第三方估算;官网与定价页实读)**
  - Stripe 位次 **41→16**,引荐访问 **11.4万→26.4万**;整站 2026-03~07 约 **0.38万→63.4万/月**。官网公开 Free / Starter **$15.99/月** / Pro **$50/月** / Studio **$200/月**,按生成 credits 售卖 3D、GUI、图标、纹理、角色、动画、音乐和代码等游戏资产生产能力。
  - 付款层是“减少 Roblox / UEFN / Unity / Unreal / Blender 的资产生产成本”,不是普通图片生成器;但 2026-07 自然词几乎全是 `forge ai`、`forge ai roblox` 及品牌拼写变体,少量 `roblox ui`,流量与通用产品词不同源。
  - **五重过滤**:同需求层 `通过`;同模式 / 同阶段 / 同验证 `失败`——完整模型工具箱、游戏生态资产库、575K+ creator 社区和品牌搜索都无法在两周内复制,本项目也没有游戏开发分发面。
  - **判定**:**不进入付费探针。** 只保留原则:免费转换层上游的“可直接投入生产的资产”能卖 credits;不能由此推导“再做一个 AI 生成器”。

- **2026-08-26 · `trustmrr.com`:可作机会数据源和曝光渠道,不可作第五站产品(数据源:支付引荐表 / SimilarWeb 关键词概览,第三方估算;官网实读)**
  - Stripe 位次 **815→484**、引荐访问 **5,948→11,145**;Lemon Squeezy 位次 **52→3**、引荐访问 **590→3,025**;整站约 **76.5万→91.8万/月**。官网核心是“验证收入的 startup 数据库 + SaaS 买卖 marketplace”,页面同时有大量赞助位。
  - 自然流量主要来自被收录 startup / 创始人的品牌词,仅少量 `acquire profitable startups`、`mrr saas`、`mrr website`;这是 UGC 数据页、收入验证和买卖双方供给共同形成的 pSEO / network flywheel。
  - **五重过滤**:需求层 `通过`(买卖线索、曝光和赞助均可收费);同模式 / 同阶段 / 同验证 `失败`——没有可信收入数据与双边供给,空目录无法复现 200K+ 月访问。
  - **判定**:**不进入付费探针。** 后续只把它当收入证据库和潜在付费曝光渠道;站内公开的 Adspirer 近 30 天收入属于 Adspirer 的辅助证据,不是 TrustMRR 自身收入。

- **2026-08-26 · `adspirer.ai` / `adspirer.com`:最值得迁移的产品形态,但广告产品本身不做(数据源:支付引荐表 / SimilarWeb 整站流量,第三方估算;官网定价与 TrustMRR 公开页实读)**
  - `.ai` 是应用端,公开营销与定价在 `.com`。Stripe 位次 **640→171**,引荐访问 **7,927→30,655**;整站 2026-03~07 约 **361→23,264/月**。TrustMRR 搜索结果公开显示近 30 天收入约 **$70,229**;该值是第三方收入验证,不等同审计报表。
  - 官网产品不是“广告创意生成”,而是通过 ChatGPT / Claude / Codex 等客户端操作 Google、Meta、Amazon、LinkedIn、TikTok、ChatGPT Ads 的 **MCP paid-media agent**:建 campaign、拉表现、诊断、批量优化。年付折算 Free / **$40 / $83 / $167 每月**,按 15 / 150 / 600 / 3,000 tasks 分层。
  - **五重过滤**:需求层与收费层 `通过`;同模式 `部分通过`(轻前端 + agent/MCP 是可迁移形态);同阶段 / 同验证 `失败`——六个平台 OAuth/API、广告操作正确性、账户风控和 paid-media 专业知识都不是当前两周 MVP。
  - **判定**:**不做广告代理克隆。** 保留可迁移模式:“让用户在现有 AI 客户端里委派高价值操作,按 task 收费”,优先迁移到本项目已有 GSC / GA4 实战与发布防错,而不是跨行做投放。

- **2026-08-26 · `app.tryatria.com`:高客单创意运营系统,不是轻量工具站样本(数据源:支付引荐表 / SimilarWeb 整站流量,第三方估算;官网与定价页实读)**
  - Stripe 位次 **627→388**,引荐访问 **8,151→13,875**;Paddle 2026-07 新出现 **31** 次;整站 2026-02~07 约 **24.8万→36.6万/月**。
  - Atria 面向品牌与 agency,把竞品研究、review mining、brief、AI 素材、Meta 批量发布、表现分析和 Slack agent 串成闭环;官网称 20K+ teams、训练于 $9B+ ad spend。年付折算 Core **$129/月**、Plus **$479/月**、Business **$959/月**,另有 Enterprise。
  - **五重过滤**:需求层 / 合规 `通过`;同模式 / 同阶段 / 同验证 `失败`——历史广告数据、Meta 集成、创意方法论、团队协同和 enterprise 服务构成壁垒,无法压成两周单人 MVP。
  - **判定**:**不进入付费探针。** 可复用的不是“再做广告 AI”,而是把洞察→动作→结果回读闭环卖给已有高价值工作流。

### 桌面筛选收口(2026-08-26)

- **直接克隆候选:0 个。** 6 个样本均证明有人付款,但没有一个同时通过同阶段、可复制分发和两周验证三关。支付引荐池完成了“找到钱流”,没有自动完成“找到我们的产品”。
- **唯一保留的付费探针命题**:`AI 建站发布防错 + 持续 SEO 巡检`——将 Outrank 的结果托管、Adspirer 的 AI-client 操作层、Atria 的洞察→动作→回读闭环,收窄到本项目已有证据的 canonical / redirect / sitemap / soft-404 / GSC / GA4 问题。
- **仍未 GO**:下一步不是开发,而是写一页具体 offer,向 15 个有 AI 建站 / SEO 发布需求的人定向触达,按本实验门槛收 **$19 refundable deposit**。在 ≥3 个有效邮箱且 ≥1 deposit 之前,不得注册域名或写第五站代码。

### 2026-08-26 补跑:哥伦布结案 + 发现「买量型」第二模型

> 触发:用户质疑筛选口径仍卡在低成本框架,并指出「很多站在买量,侧面证明这门生意赚钱」。
> 数据源:Columbus MCP(快照 2026-07,**第三方估算**)+ gefei-kd MCP(**第三方估算**)。

**一、`ai ad creatives` 结案 —— 哥伦布最后一个高 CPC 词是幻觉**

全库唯一可建站的通用高 CPC 词($25.46 / 36,460),拆到底:

- 命中站仅 3 个,其中 2 个自然搜索 **0%**(按硬边界 4 出局),真正的样本只有 `nextify.ai`
- **`nextify.ai` 从这个词拿到的估值只有 $650/月 ≈ 26 次点击。** 词面 36,460,实际捕获 26。
  这个数字与预算无关 —— 花 $6–18K 买链去抢一个月吐 26 次点击的词,钱多也不该这么花
- gefei-kd:KD **57.2**,进前十 **80–170 引用域**(中值 120,对应 DR 35);若以目录型外链为主需 **290–700**。
  `nextify.ai` 实测 439 引用域 / dofollow 占比仅 43%,坐实后一个区间
- `nextify.ai` 本体三条已立案模式全中:`siteType=brand`、`isAllInOne=true`、
  `seo_playbook=[model_pages, alternatives_pages]` 且第二大词 `gethookd ai` 是别人的品牌名。
  10.4 万月访问,停留 **26 秒**、体验分 22/100、美国占比 8.1%、支付方检测为空

**二、全库 374 词复核 —— 我提的「单站词口子」证伪**

08-25 普查用了 `min_frequency=2`,我判断它排除了单站词、结论未封顶。去掉全部下限后
`totalMatched` 370 → **374**,只多 4 个词,CPC 天花板完全一致(库本身只收录 ≥3 站命中的词)。
**08-25 封顶结论成立,是我错。**

但这次补到了 08-25 漏记的三个字段(`kd` / `intent` / `estimateBacklink`),
用它们重筛,确有几个词是旧筛子(KD<25)自动毙掉、08-12 新约束下够得着的:

| 词 | 月搜 | CPC | KD | 进前十链接 |
|---|---|---|---|---|
| `oc maker` | 14,600 | $2.17 | 50 | 84 |
| `ai video detector` | 6,040 | $1.95 | 45 | 68 |
| `ai voice detector` | 4,720 | $1.97 | 40 | 56 |

**但算完账仍不成立**:`oc maker` 打进前三约 3,000–4,000 UV → 广告变现 $10–30/月,
而 84 个引用域按 $50–100/条是 $4,200–8,400,**回本 4–35 年**。
这印证 08-13 自己写的方法论:**换参数不等于换池子,该换的是样本库本身。**

**三、真正的新发现:`sem=high` 桶里有一个完全不同的模型**

全库 4,630 站中 `sem=high` 仅 **118 个(2.5%)**,买量是少数派。但深拆 10 个后,
这 2.5% 内部还分成**相反的两型**(完整 10 站表见 [sources/columbus.md](sources/columbus.md)):

| | **A 型:品牌词截流套利** | **B 型:正常生意 + 付费补量** |
|---|---|---|
| 样本 | `aimusicai.co` / `voice2texts.com` / `yumepik.com` / `wann.ai` / `whispertranscribe.ai` | `datephotos.ai` / `coprep.ai` / `hinoter.com` |
| 付费占比 | **59–93%** | **12–16%** |
| 自然搜索 | 0.02%–13% | **28%–47%** |
| 定价线索 | 全是 `Free` 或空 | **完整价格阶梯** |
| 首页字数 | 239 / 368 / 812 / 825 / 1387 | 1293 / 3209 / 3286 |
| 稳定性 | 5 个里 2 个在跌(-37% / -13.7%);地理极端集中(`aimusicai` **99.99% 日本**) | 站龄更长,自然搜索为主体 |

- **A 型的 CPC 便宜有具体原因:买竞品和模型的品牌词。**
  `suno` 3,049,530 / **$0.23**、`suno ai` 1,514,380 / **$0.18**、`higgsfield` 3,368,900 / **$0.36**、
  `whisper` 351,810 / **$0.77**。比 `csv to qbo` 的 $7–15 便宜 **10–80 倍** —— 这就是 CAC 数学成立的地方。
  **但这个便宜只因为品牌方还没来抢自己的词,窗口由第三方单方面控制**(准公理 B)。
- **入场券远超预算(我的推算,非读数)**:按付费访问 × 均价 CPC 倒推,
  `yumepik` ≈ **$2.6 万/月**、`whispertranscribe` ≈ **$5.7 万/月**、`voice2texts` ≈ **$11.2 万/月**。
  不是独立开发者量级。
- **B 型才是可攒的形态**,尤其 `datephotos.ai`:9 个月、DR 33、47.2% 自然搜索、
  价格阶梯 **$29–800**、`programmatic_seo` + `affiliate_program` + 533 引用域 49% dofollow。
  **它跟自有四站打法同源,只是变现层做对了。**

**四、对「个人开发者做不动出海 web」这个判断的实证反驳**

本轮 10 站里,注册 4–12 个月、DR 0–33 就做到几万至几十万月访问的有 8 个:

| 站 | 站龄 | 月访问 | DR | 自然搜索 | 定价 |
|---|---|---|---|---|---|
| `wann.ai` | 4 个月 | 87,230 | 1.9 | 3.8% | — |
| `senzia.cc` | 7 个月 | **380,681** | **0** | 35.0% | — |
| `solmi.ai` | 8 个月 | 67,423 | 25 | 36.1% | — |
| `datephotos.ai` | 9 个月 | 28,914 | 33 | **47.2%** | **$29–800** |
| `whispertranscribe.ai` | 9 个月 | 181,200 | 2.2 | 13.4% | — |
| `yumepik.com` | 11 个月 | 147,391 | **0** | 3.0% | — |
| `hinoter.com` | 11 个月 | 98,751 | 3.3 | 28.0% | **$2 / $600** |
| `coprep.ai` | 12 个月 | 48,482 | 29 | **41.6%** | **$12–72/月** |

**结论:七轮筛选全部死在变现层,零轮死在「做不出来」或「排不上去」,
零轮死在「试了但没人付钱」—— 因为从未走到那一步。** 截至今日 4 站、十几个词 Top 10、
**从未完成过一次收款**。真正的约束是四条具名项(18h/周 / 无 Stripe 资质 /
**零分发面** / $2–5K 预算定了 14 天未花一分),不是「个人开发者不行」。

**五、下一步(未 GO,仍不批准建站)**

- A 型不抄:买别人商标词的位置,风险与窗口都不在自己手上
- 若要验 A 型的核心未知(**转化率**):$500 × CPC $0.20 = 2,500 点击。
  1% 转化到 $10/月订阅 = 25 订户 = $250/月,两个月回本。**这个数买到的是真值,不是第三方估算**
- **但前置未决**:08-12 定的 $2–5K 预算是否真能花 —— 未确认前该测试跑不动
- 选题优先拆 **B 型模板**:明确场景 + 客单 $29 起 + 自然搜索能承接 + programmatic SEO

### 2026-08-27 · Image-to-3D 赛道评估(外部交接立项 → **判定 NO-GO,不作第五站**)

> 触发:另一 agent 交接了一份「image-to-3D 赛道选词立项」,结论是「赛道对新站友好、5 个切入词 KD 23–45 容易打」,
> 唯一卡点标为「搜索量未核实」。本轮把卡点补上,顺带复核了赛道判断。
> 数据源:SimilarWeb API 直连(`fetch-keyword-overview-bundle`,country=999 全球,月份 2026-07)+ Columbus MCP(快照 2026-07)
> + gefei-kd MCP,**全部第三方估算**,拉取日期 2026-08-27。
> **未动用 Ahrefs**——Columbus 的引用域字段与 2026-08-15 的 Ahrefs 实测口径吻合(见下方「三口径」段),省掉一次计费调用。

**一、P0 已解:搜索量存在,但 1/5 的词是死的**

| 词 | 2026-07 | 近 12 月均 | 前 12 月均 | YoY | 零点击 |
|---|---|---|---|---|---|
| `image to 3d model`(交接已排除) | 46,710 | 54,922 | 39,309 | **+39.7%** | 47.1% |
| `ai 3d model generator`(交接已排除) | 46,610 | 46,011 | 41,837 | +10.0% | 43.1% |
| `photo to 3d model` | 8,440 | 9,757 | 8,701 | +12.1% | 48.0% |
| `picture to 3d model` | 6,520 | 7,322 | 6,418 | +14.1% | **57.4%** |
| `2d to 3d model` | 3,560 | 3,529 | 3,137 | +12.5% | 52.1% |
| `turn image into 3d model` | 3,180 | 2,621 | 1,859 | **+41.0%** | 36.8% |
| `image to 3d model for 3d printing` | **10** | **112** | 35 | — | — |

- **需求真实且在涨**,不是被通用大模型吸收的品类(对照[细分 AI 生成需求被通用大模型吸收](risks.md))。
- 但**交接主推的「三个内容真空词」合计仅 ~19.7K/月全球**;按 `ai3dgen.com` 美国占比 23% 折算,**美区 ~4–5K/月**。
- `image to 3d model for 3d printing` **应立即弃词**:2026-07 全球 10 次搜索,SERP 组成全 0。交接把它列为「用途词离钱近」是错的。
- 全部 7 个词 `AioExistence=1`、意图 **100% informational**、零点击 37–57%。

**二、KD 三口径 2:1 否掉「容易」这个前提**

| 词 | gefei-kd | SimilarWeb | Columbus |
|---|---|---|---|
| `picture to 3d model` | **28.1**(08-27 复跑;交接记的 23.2 已漂移) | **64** | — |
| `photo to 3d model` | 28.1(交接) | **71** | — |
| `turn image into 3d model` | 32.9(交接) | **83** | — |
| `2d to 3d model` | 37.5(交接) | **53** | — |
| `image to 3d model free` | — | — | **62**(链接预算 141) |

按 [seo-competitor 边界 3](../.claude/skills/seo-competitor/SKILL.md),**差 ≥2 倍即为分歧标记,真值只能靠 GSC 单页实测**。
交接文档只取了最乐观的那一家。

**三、最硬的一条:实测引用域比链接预算估算高 10–24 倍**

gefei-kd 说进前十约 25–50 个引用域(中值 35)。实际在排的站(Columbus,2026-07):

| 站 | 引用域 | dofollow% | ≈dofollow 域 | 站龄 | 自然搜索占比 |
|---|---|---|---|---|---|
| `hi3d.ai` | **2,080** | 68% | ~1,414 | 6 个月 | 13.8% |
| `magic3d.io` | **1,203** | 82% | ~986 | 10 个月 | 64.9% |
| `ai3dgen.com` | 633 | 18% | ~114 | 11 个月 | 66.4% |
| `fast3d.io` | 516 | 44% | ~227 | 15 个月 | 59.4% |
| `supavoxel.com` | 486 | 8% | ~39 | 7 个月 | 35.0% |

- 靠自然搜索真正起来的最弱样本是 `ai3dgen.com` **~114 个 dofollow 域**,不是 35。
- **交叉验证顺带确认了 Columbus 引用域字段可用**:`magic3d.io` 本轮 1,203×82%≈**986 dofollow**,
  与 [2026-08-15 Ahrefs 实测 902 dofollow](#结果记录) 差 <10%。**以后查引用域可用 Columbus,不必动 Ahrefs。**
- 这是[顾问外链门槛系统性偏低](risks.md#顾问外链门槛估算系统性偏低把链接预算当可执行数字方法论错误)**第三次**,倍差从 2.4–5.4x 扩大到 **10–24x**。

**四、交接的赛道判断有三处硬错**

1. **样本不全(同[筛选上限自蔽](risks.md#筛选上限自蔽用带上限的结果判断赛道竞争方法论错误))**:交接列了 4 个站,
   Columbus `image-to-3d-model` 子类收录 **38 个**。真正头部是 `hi3d.ai` —— **注册 2026-01-30,6 个月做到 175.5 万月访问**,交接完全没看到。
2. **`magic3d.io DR 5` 是误读**:DR 4.9 属实,但它堆了 **1,203 引用域 / 82% dofollow**。这不是「低权重新站也能赢」,
   是「近千个 dofollow 域但 DR 算法没给分」。**同一个站在 2026-08-15 已因同样原因被记进 risks.md 作为数据失真样本,这次又被当成正面证据。**
3. **`fast3d.io` 是反面证据**:2026-03 峰值 **265,593** → 2026-07 **150,101**,**4 个月 -43%**,trend=down。

**五、品类形态 = 高流失脉冲(准公理 A)**

38 个站里 2026-07 环比为负的有 **12 个**:`hitem3d.ai` **-93%**(已 301 跳到 `hi3d.ai`,5 月还有 112 万)、
`formy3d.com` -56%、`three.ws` -45%、`project-genie.ai` -43%、`hy-image.org` -37%、`sam-3d.com` -36%、
`magic3d.io` -21%、`depthflow.io` -20%、`fast3d.io` -19%、`home-design.ai` -19%、`gen3d.pro` -19%、`modelfy.art` -9%。

**六、变现:与前四站不同型,但仍不成立**

| 站 | 定价线索 | 月访问 | 自然搜索占比 |
|---|---|---|---|
| `hi3d.ai` | **Free** + API | 1,754K | 13.8% |
| `ai3dgen.com` | Free, **$3** | 264K | 66.4% |
| `supavoxel.com` | $9 / $19 / $29 / $99/mo + $0.048 credits | 166K | **35.0%** |
| `fast3d.io` | **Free 只此一档** | 150K | 59.4% |
| `magic3d.io` | $9.9 / $19.9 / $39.9 / $129.9 + 积分包 | 101K(**在掉**) | 64.9% |

**决定性读数:自然搜索占比最高的两个站(ai3dgen 66%、fast3d 59%),定价是 $3 和纯 Free。**
有像样价格阶梯的 `supavoxel` 只有 35% 自然搜索,`magic3d` 在掉。CPC $0.74–1.71,**低于 [$2 闸门](risks.md#ai-工具站品类的-cpc-系统性低-1-2-个数量级用它找付费生意是池子选错违反公理-4)**。
用户还在搜索框里直接写 free:`image to 3d model free` **12,530/月**,是 `picture to 3d model` 的近 2 倍。

**七、与前四站的真正差别(必须说清,否则会误归因)**

partfit3d / aidepixelate 死于[做在被定价为 0 的那一层](risks.md#做在被定价为-0-的那一层四站-0-收入的统一解释违反公理-4--准公理-b):
零边际成本 / 被市场定位为廉价替代。**image-to-3D 不是那一型**——每次生成烧 GPU(正边际成本),
而且钱确实在这一层(`3daistudio` 自报 ARR $3.3M)。

**问题换成了另一个:这一层的价格由开源模型定。** 看 Columbus 这批站的名字就够了——
`hy-3d.net` / `hy3d.dev`(腾讯 Hunyuan3D,**Apache 2.0,6GB 显存本地可跑**)、`trellis2.app` / `trellis3.com`(微软 Trellis)、
`sam-3d.com`(Meta SAM 3D)。**模型免费开源 → 套壳零护城河;而你要替用户付 GPU 钱。**
净形态 = **零护城河 + 正边际成本 + CPC < $2 + 用户自带 `free` 意图**。

**八、对三道门的判定**

| 门 | 判定 |
|---|---|
| ① 核心词有付费意愿 | **不过** —— CPC $0.74–1.71 < $2;自然搜索赢家定价 $3 / Free |
| ② 做在市场单独付费的层级 | **矛盾** —— 这层有人付钱(supavoxel/magic3d 有阶梯),但付钱的那批不靠自然搜索,靠自然搜索的那批收不到钱 |
| ③ 2 周内可验证付费 | 过 |

**→ NO-GO,不作第五站主选题,不注册域名。**

**九、唯一值得单独留的一条(不是这个赛道,是这个样本)**

`supavoxel.com`:2026-01-09 注册,7 个月 166K 月访问,**$9/$19/$29/$99 完整价格阶梯 + $0.048 按次积分**,
tags 含 `programmatic_seo` / `public_api` / `user_reviews`,**dofollow 引用域仅 ~39 个**。
这是本轮 38 个站里唯一同时具备「真实价格阶梯 + 低外链门槛」的样本,
形态与正在拆的 [`datephotos.ai` B 型模板](todos.md)同类。**拆形态,不克隆品类。**

**十、本轮未做**

- **Google Trends 的 Rising 相关查询列表未拉**。按 [risks.md 该条纪律](risks.md#细分-ai-生成需求被通用大模型吸收用-trends-rising-列表当读数违反准公理-b)本应在查 KD 前先看,
  本轮先用 SimilarWeb 24 个月量史替代(结论:在涨,未被吸收),**但「涨去了哪里」这一问没答**。若日后重启该方向,补这一步。
- 美区(country=840)分国口径未单独拉,美区量按 `ai3dgen` 国别占比折算,**是估算不是读数**。
...
- **时间盒**:~~2 周,至 2026-09-08~~ → **2026-08-26 顺延至 09-13**。
  原时间盒物理上达不到:Step 3b 最早 08-30 上线(要等 TDK 读数),Step 4 要求 Step 3b 上线后重锁 14 天窗口 → 08-30+14 = 09-13。
  **GO/NO-GO 标准不变**——缩短窗口会让 CTR 读数不可信。
- **预算上限**:0 元
...
| 砍模型词域名站 | ~17 | [risks.md 已立条目](risks.md#模型词域名站入口靠抢时间留存靠改品牌两头都不占违反准公理-a) |
| 砍负增长 + NSFW | **11** | — |
