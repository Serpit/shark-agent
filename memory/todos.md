...
- [x] **2026-08-27 完成** · M5 前置 · 评估 image-to-3D 赛道(外部 agent 交接立项)—— **判定 NO-GO**
  - **P0 已解**:5 个切入词搜索量全部拿到(SimilarWeb 直连,全球口径)。4 个有量(2.6K–9.8K/月),
    `image to 3d model for 3d printing` **是死词**(2026-07 全球 10 次搜索),应立即弃
  - **但推翻了交接的核心前提**:KD 三口径 2:1 否掉「容易」(gefei 28 vs SimilarWeb 64–83);
    实测引用域 486–2,080,是链接预算估算的 **10–24 倍**;交接漏了真正的头部 `hi3d.ai`(6 个月 175 万月访问);
    `magic3d.io DR 5` 是误读(1,203 引用域 / 82% dofollow);`fast3d.io` 4 个月 -43% 是反面证据
  - **否决理由(三道门)**:CPC $0.74–1.71 < $2 闸门;自然搜索占比最高的两个站定价 $3 / 纯 Free;
    上游模型开源(Hunyuan3D Apache 2.0,6GB 显存本地可跑)→ 零护城河 + 正边际成本
  - **沉淀**:[risks.md 新条目「开源模型套壳」](risks.md) + [顾问外链门槛低估第三次记录](risks.md)
  - **顺带结论**:Columbus 的引用域字段与 Ahrefs 实测差 <10%,**以后查引用域不必动 Ahrefs**(另计费)
  - **关联**:[experiments.md 2026-08-27 赛道评估](experiments.md#2026-08-27--image-to-3d-赛道评估外部交接立项--判定-no-go不作第五站)

- [ ] **2026-08-27**(承接上条,与 `datephotos.ai` 合并做)· M5 前置 · 把 `supavoxel.com` 加进 B 型形态模板拆解
  - **为什么**:image-to-3D 那 38 个站里唯一同时具备「真实价格阶梯 + 低外链门槛」的样本 ——
    2026-01-09 注册、7 个月 166K 月访问、**$9/$19/$29/$99 + $0.048 按次积分**、
    `programmatic_seo` + `public_api` + `user_reviews`,**dofollow 引用域仅 ~39 个**
  - **动作**:人工开站,与 `datephotos.ai` 并排拆四件事 ①价格阶梯怎么分档 ②按次积分和订阅怎么共存
    ③programmatic SEO 生成的是什么页 ④只有 39 个 dofollow 域是怎么做到 166K 的(**这一条最值钱**)
  - **⛔ 边界**:拆的是形态,**不是克隆 image-to-3D 品类**——该品类已判 NO-GO
  - **预计耗时**:1 小时(与 `datephotos.ai` 那条合并跑)
...
- [ ] **2026-08-28** · M5 前置 · 用 Fiverr 测 `ghost mannequin` 的付费意愿(**先测钱,不建站**)
  - **为什么排进来**:拆 [enhancecraft.com](findings/2026-08-28-teardown-enhancecraft.md) 时挖出的方向,
    是 08-26 定的三道新判据下**第一个三门全过的候选** —— CPC 上限 $7.74(过 $2 闸门)、
    人工服务按张收 $16–30(不在被定价为 0 的层)、Fiverr 有成熟品类(2 周内可验)
  - **可打性佐证**(gefei-kd):难度 26.8/100、月搜 4,310、进前十 20–45 引用域;
    前十第 2 名 `adworker.ai` **域龄 6 个月 / DR 3** —— 与 `stl repair` 同量级门票
  - **动作**:挂一个 ghost mannequin 修图服务(手工 + AI 辅助),定价 $3–5/张,14 天看有没有真实订单
  - **判定线**:有单 → 付费意愿成立,再决定要不要做工具站;**0 单 → 这个词的量在信息意图上,直接停,不建站**
  - **⛔ 边界**:买家是服装电商卖家,与 X / 独立开发圈不重叠,Build in Public 带不动 —— 这是已知代价,不是否决
  - **关联**:[themes.md 电商/房产工序型图像处理](themes.md) · [risks.md 08-28 CPC 第二条限定](risks.md)
  - **预计耗时**:挂单 1 小时 + 14 天等待
